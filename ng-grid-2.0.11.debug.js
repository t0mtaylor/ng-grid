...
(function(window, $) {
	...
	var	ngGridServices = angular.module('ngGrid.services', []),
		ngGridDirectives = angular.module('ngGrid.directives', []),
	...
	var ngMoveSelectionHandler = function($scope, elm, evt, grid) {
		...
		if ($scope.enableCellSelection) {
			...
			var focusedOnFirstColumn = $scope.showSelectionCheckbox ? $scope.col.index === 1 : $scope.col.index === 0,
				focusedOnFirstVisibleColumns = $scope.$index === 1 || $scope.$index === 0,
				focusedOnLastVisibleColumns = $scope.$index === ($scope.renderedColumns.length - 1) || $scope.$index === ($scope.renderedColumns.length - 2),
	...
	ngDomAccessProvider.prototype.selectionHandlers = function ($scope, elm) {
		...
		function keydown (evt) {
			if (evt.keyCode === 16) { //shift key
				...
				doingKeyDown = true;
				doingKeyDown = false;
				return ngMoveSelectionHandler($scope, elm, evt, self.grid);
			}
			return true;
		}
	...
	var ngFooter = function ($scope, grid) {
		$scope.maxRows = function () {
			return Math.max($scope.totalServerItems, grid.data.length);
		};
	...
	var ngGrid = function ($scope, options, sortService, domUtilityService, $filter, $templateCache, $utils, $timeout, $parse, $http, $q) {
			...
			if (columnDefs.length > 0) {
				...
				angular.forEach(columnDefs, function(colDef, i) {
					...
					var column = new ngColumn({
						...
						indx = self.config.groups.indexOf(colDef.field);
					if (indx !== -1) {
		...
		self.configureColumnWidths = function() {
			...
			if (percentArray.length > 0) {
				...
				var percentWidth = 0, // The total % value for all columns setting their width using % (will e.g. be 40 for 2 columns with 20% each)
					hiddenPercent = 0; // The total % value for all columns setting their width using %, but which have been hidden
		...
		self.fixColumnIndexes = function() {
			//fix column indexes
			var i = 0,
				columnsLen = $scope.columns.length;
			for (; i < columnsLen; i++) {
		...
		$scope.adjustScrollLeft = function (scrollLeft) {
			...
			if(repaint) {
			  ...
			  if (dcv)
				...
				var xp = $scope.columns_pinned.length;
				for (; i < xp; i++) {
					col = $scope.columns[i];
					newLeft = i > 0 ? (scrollLeft + totalLeft) : scrollLeft;
					domUtilityService.setColLeft(col, newLeft, self);
	...
	var ngRowFactory = function (grid, $scope, domUtilityService, $templateCache, $utils) {
		...
		self.renderedChange = function() {
			...
			var rowArr = [],
				i = self.renderedRange.topRow,
				bottomRow = self.renderedRange.bottomRow;
			for (; i < bottomRow; i++) {
				if (temp[i]) {
	...
	var ngSearchProvider = function ($scope, grid, $filter) {
		...
		var searchColumn = function(condition, item){
			var result,
				col = self.fieldMap[condition.columnDisplay];
			if (!col) {
				return false;
			}
			var sp = col.cellFilter.split(':'),
				filter = col.cellFilter ? $filter(sp[0]) : null,
				value = item[condition.column] || item[col.field.split('.')[0]];
		...
		self.evalFilter = function () {
			...
			var i = 0,
				frLength = grid.filteredRows.length;
			for (; i < frLength; i++) {
				grid.filteredRows[i].rowIndex = i;
			}
					...
					if (col.field) {
						if(col.field.match(/\./g)){
							var properties = col.field.split('.'),
								currentProperty = self.fieldMap,
								j = 0,
								propsLen = properties.length - 1;
							for(; j < propsLen; j++) {
	...
	var ngSelectionProvider = function (grid, $scope, $parse) {
		...
		self.ChangeSelection = function (rowItem, evt) {
			...
			var charCode = evt.which || evt.keyCode,
				isUpDownKeyPress = (charCode === 40 || charCode === 38);
			...
			compile: function() {
				return {
					pre: function($scope, iElement) {
						var html,
							cellTemplate = $scope.col.cellTemplate.replace(COL_FIELD, 'row.entity.' + $scope.col.field);
	...
	ngGridDirectives.directive('ngInput', [function() {
		return {
			...
			link: function (scope, elm, attrs, ngModel) {
				...
				var oldCellValue,
					dereg = scope.$watch('ngModel', function() {
