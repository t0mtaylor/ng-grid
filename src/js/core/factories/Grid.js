...
/**
  ...
  Grid.prototype.updateDrawnColumnWidths = function () {
    var asterisksArray = [],
      percentArray = [],
      manualArray = [],
      asteriskNum = 0,
    ...
    if (typeof(self.verticalScrollbarWidth) !== 'undefined' &&
      self.verticalScrollbarWidth !== undefined &&
      self.verticalScrollbarWidth > 0) {
      availableWidth = availableWidth + self.verticalScrollbarWidth;
    }
    ...
    if (leftoverWidth > 0 && canvasWidth > 0 && canvasWidth < availableWidth) {
      ...
      self.renderableColumns.forEach(function (col) {
      ...
      if (variableColumn) {
        ...
        while (leftoverWidth > 0) {
          self.renderableColumns.forEach(remFn);
        }
      }
    }
    ...
    if (self.verticalScrollbarWidth) {
      canvasWidth = canvasWidth + self.verticalScrollbarWidth;
    }
    // canvasWidth = canvasWidth + 1;
  ...
  Grid.prototype.getColumnStyles = function(){
    var ret = '';
    var self = this;
    self.renderableColumns.forEach(function (column) {
      ret = ret + ' .grid' + self.id + ' .col' + column.index + ' { width: ' + column.drawnWidth + 'px; }';
    });
    return ret;
  };
